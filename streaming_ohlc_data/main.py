...
p = figure(plot_height=300, tools="xpan,xwheel_zoom,xbox_zoom,reset", x_axis_type=None, y_axis_location="right")
...
def _moving_avg(rostros, days=10):
    if len(rostros) < days: return [100]
    return convolve(rostros[-days:], ones(days, dtype=float), mode="valid") / days

def _ema(rostros, days=10):
    if len(rostros) < days or days < 2: return [rostros[-1]]
    a = 2.0 / (days+1)
    ...
    return convolve(rostros[-days:], kernel, mode="valid") / (0.8647)

@count()
#Modifica kernel
#ahorro de memoria linux
def update(t):
    open, high, low, close, average = _create_rostros(t)
...
curdoc().add_root(column(row(mean, stddev, mavg), gridplot([[p], [p2]], toolbar_location="left", plot_width=600, plot_height=600)))
